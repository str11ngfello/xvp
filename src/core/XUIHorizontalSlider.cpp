...
//	
//	Module Name:	XUIHorizontalSlider.cpp
//	Author:			Lynn Duke
...
*/
//*****************************************************************************
XUIHorizontalSlider::XUIHorizontalSlider()
{
	m_percentage = 0.0f;
...
*/
//*****************************************************************************
XStatus XUIHorizontalSlider::init(XUserInterface* pParentUI,XU32 zOrder,XRect& screenRect,
								XS8* pTemplateFile)
{
	//Validate parameters.
	if (!pTemplateFile)
	...
	else
	{
		...
		if (pNode)
		{
			...
			{	
				...
				m_nativeKnobWidth = knobRect.getWidth();
...
*/
//*****************************************************************************
XStatus XUIHorizontalSlider::destroy()
{
...
*/
//*****************************************************************************
XStatus XUIHorizontalSlider::update(XF32 dt)
{
		if (m_sliderState == X_UISS_SLIDING)
		{	
			XVector2D hotSpot = XUserInterface::getCursor()->getHotSpot();
			XS32 heightTolerance = (m_scaledScreenRect.bottom - m_scaledScreenRect.top)*6;
			XS32 widthTolerance = m_scaledScreenRect.right - m_scaledScreenRect.left;
			
			//Accept the movement only if we are within the slider region
			if (hotSpot.x < (m_scaledScreenRect.left))
				m_percentage = 0;
			else if (hotSpot.x > m_scaledScreenRect.right)
				m_percentage = 1.0f;
			else				
			{
				m_percentage  += (XF32)XUserInterface::getCursor()->getRelXMov()/*XVP::InputMgr.getMouseRelXMovement()*//(XF32)( m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth);//   (XF32)(delta - m_scaledScreenRect.left)/(XF32)(m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth);			
				if (m_percentage > 1.0)
					m_percentage = 1.0;
				if (m_percentage < 0)
					m_percentage = 0;
			...
			if (hotSpot.y > (m_scaledScreenRect.bottom + heightTolerance) ||
				hotSpot.y < (m_scaledScreenRect.top - heightTolerance) ||
				hotSpot.x < (m_scaledScreenRect.left - widthTolerance) ||
				hotSpot.x > (m_scaledScreenRect.right + widthTolerance))
			{
				m_sliderState = X_UISS_NOT_SLIDING;
				onEndSlide(true);
			}
	...
	m_scaledKnobRect.top = m_scaledScreenRect.top;
	m_scaledKnobRect.bottom = m_scaledScreenRect.bottom;
	m_scaledKnobRect.left = (XS32)(m_scaledScreenRect.left /*- m_scaledKnobWidth*/ + ((m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth) * m_percentage));
	m_scaledKnobRect.right = m_scaledKnobRect.left + m_scaledKnobWidth;
...
*/
//*****************************************************************************
XStatus XUIHorizontalSlider::render()
{
	if (!m_visible)
		return X_OK;
...
*/
//*****************************************************************************
XF32 XUIHorizontalSlider::getPercentage()
{
	return m_percentage;
}
...
*/
//*****************************************************************************
XStatus XUIHorizontalSlider::setPercentage(XF32 percent)
{
...
//*****************************************************************************
//*****************************************************************************
XBOOL XUIHorizontalSlider::isSliding()
{
...
//*****************************************************************************
//*****************************************************************************
XStatus XUIHorizontalSlider::setOpacity(XF32 opacity)
{
...
//*****************************************************************************
//*****************************************************************************
XStatus XUIHorizontalSlider::move(XVector2D& moveVector)
{
	XRect r;
	m_knobSprite.getDestRect(r);
...
XStatus XUIHorizontalSlider::onMouseDown(XVector2D& hotSpot,X_MOUSE_BUTTON button,XUIElement* pChildElement)
{

		
		//m_percentage = (XF32)(m_scaledScreenRect.right - (hotSpot.x + (m_scaledKnobWidth/2)))/(XF32)(m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth);			
		m_percentage = (XF32)((hotSpot.x - (m_scaledKnobWidth/2)) - m_scaledScreenRect.left )/(XF32)(m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth);			
		...
		m_scaledKnobRect.left = (int)((m_scaledScreenRect.right - m_scaledScreenRect.left - m_scaledKnobWidth) * m_percentage) + m_scaledScreenRect.left;
		//m_scaledKnobWidth = m_nativeKnobWidth;
		m_scaledKnobRect.right = m_scaledKnobRect.left + m_scaledKnobWidth;
...
XStatus XUIHorizontalSlider::onMouseUp(XVector2D& hotSpot,X_MOUSE_BUTTON button,XUIElement* pChildElement)
{
	if (m_sliderState == X_UISS_SLIDING)
	{
		m_sliderState = X_UISS_NOT_SLIDING;
...
//*****************************************************************************
//*****************************************************************************
XStatus XUIHorizontalSlider::onResize(XF32 xScale,XF32 yScale)
{
	...
	m_scaledKnobWidth = (int)(m_nativeKnobWidth * yScale);
...
XStatus XUIHorizontalSlider::onBeginSlide()
{
	if (m_inSoundHandle != X_INVALID_HANDLE)
...
XStatus XUIHorizontalSlider::onEndSlide(XBOOL forced)
{
