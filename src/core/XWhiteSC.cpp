...
//*****************************************************************************
//*****************************************************************************
XStatus XWhiteSC::init(XS8* pShaderName)
{
	//Create vertex and pixel shaders necessary for rendering particle systems.
	XStatus status = X_OK;
	
	status = XVP::GraphicsMgr.createVertexShader("White.fx","mainVS",m_vsHandle);
	if (status == X_OK)
	{
		status = XVP::GraphicsMgr.createPixelShader("White.fx","mainPS",m_psHandle);
		m_inited = true;
	}
...
//*****************************************************************************
//*****************************************************************************
XStatus XWhiteSC::render(XRenderEvent* pRenderEvent)
{
	//Set shaders
