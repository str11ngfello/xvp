...
namespace Utilities
{
	...
	public class TreeViewMS : System.Windows.Forms.TreeView
	{
		...
		{
			//Clear the base classe's SelectedNode
			//as well as the SelectedNodes array list added in this derived class
			SelectedNode = null;
...
//
		...
		{
			//There's no way to tell when nodes are removed from a tree.  Since
			//MS didn't provide a mulitple selection tree view control, we 
			//use a try here to catch the case where dangling references
			//are left behind after removing nodes from the tree.  
			try
			{
			...
			{
				//Handle this gracefully by clearing the selection
				m_coll.Clear();
				SelectedNodes.Clear();
			}
		}
